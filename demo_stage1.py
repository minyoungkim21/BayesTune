...
class RunConfiguration(zconf.RunConfig):
    ...
    min_train_steps = zconf.attr(type=int, default=0)
...
def run_simple(args: RunConfiguration, with_continue: bool = False):
    ...
    with distributed.only_first_process(local_rank=args.local_rank):
        ...
        if not os.path.exists(os.path.join(model_cache_path, hf_config.model_type)):
            ...
            export_model.export_model(hf_pretrained_model_name_or_path = args.hf_pretrained_model_name_or_path,
                output_base_path = os.path.join(model_cache_path, hf_config.model_type),)

        phase_task_dict = { "train": args.train_tasks,"val": args.val_tasks,"test": args.test_tasks, }
        for task_name in full_task_name_list:
            phases_to_do = []
            for phase, phase_task_list in phase_task_dict.items():
                if task_name in phase_task_list and not os.path.exists(os.path.join(args.exp_dir, "cache", hf_config.model_type, task_name, phase)):
            ...
            my_tokenize_and_cache.main(
                my_tokenize_and_cache.RunConfiguration(
    ...
    if args.do_test:
        for tsk in jiant_task_container_config['task_cache_config_dict']:
            jiant_task_container_config['task_cache_config_dict'][tsk]['test_labels'] = \
                jiant_task_container_config['task_cache_config_dict'][tsk]['val_labels'].replace("/val_labels", "/test_labels")
            jiant_task_container_config['task_cache_config_dict'][tsk]['train_labels'] = \
                jiant_task_container_config['task_cache_config_dict'][tsk]['val_labels'].replace("/val_labels", "/train_labels")
    os.makedirs(os.path.join(args.exp_dir, "run_configs"), exist_ok=True)
    jiant_task_container_config_path = os.path.join(args.exp_dir, "run_configs", f"{args.run_name}_config.json")
    ...
    if args.model_weights_path:
        ...
        model_weights_path = os.path.join(model_cache_path, hf_config.model_type, "model", "model.p")
    run_output_dir = os.path.join(args.exp_dir, "runs", args.run_name)

    run_args = my_runner.RunConfiguration(
        ...
        model_config_path = os.path.join(model_cache_path, hf_config.model_type, "model", "config.json"),
    ...
    run_args.user_mode = args.user_mode
    run_args.min_train_steps = args.min_train_steps
    run_args.log_dir = args.log_dir
    run_args.do_test = args.do_test
    
    my_runner.run_loop(args=run_args, checkpoint=checkpoint)
